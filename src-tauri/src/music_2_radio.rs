...
pub fn to_radio(file_path: &str, song_name: String) {
    ...
    let output_path = format!("{}/{}", cur_dir.to_str().unwrap(), OUTPUT_FOLDER);

    if fs::metadata(output_path.as_str()).is_err() 
    {
        fs::create_dir(output_path.as_str()).unwrap();
    }

    let mut write_radio = WavWriter::create(
        format!("{}/radio_{}.wav", output_path.as_str(), song_name),
